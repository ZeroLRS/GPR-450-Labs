	...
	if (clipCtrl && clipCtrl->clipListBasePtr_pool)
	{
		...
		while (solving)
		{
			if (clipCtrl->keyframeTimeParam >= 1)
			{
				...
				{
					nextIndex = clipCtrl->clipPtr->keyframeFirst;
				}
		...
		// update parameter
		clipCtrl->keyframeTimeParam = clipCtrl->keyframeTime 
			* clipCtrl->keyframePtr->durationInv;
...
inline a3i32 a3clipControllerSetClip(a3_ClipController* clipCtrl, const a3_ClipPool* clipPool, const a3ui32 clipIndex_pool)
{
	if (clipCtrl && clipPool && clipPool->clip && clipIndex_pool < clipPool->count)
	{
		...
		a3clipController_internalSetKeyframe(clipCtrl, clipCtrl->clipPtr->keyframeFirst);
